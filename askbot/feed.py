...
from askbot.models.space import resolve_space_or_404
...
class RssIndividualQuestionFeed(Feed):
    ...
    def get_object(self, request, space_slug, pk):
        if not askbot_settings.RSS_ENABLED:
            raise Http404
        # hack to get the request object into the Feed class
        self.request = request
        post = Post.objects.get_questions().get(id__exact=pk)

        space, redirect = resolve_space_or_404(space_slug)

        if space.pk != post.thread.space_id:
            raise Http404

        self.space = space

        if redirect:
            params = {'space_slug': space.slug, 'id': post.pk}
            redirect_url = reverse('individual_question_feed', kwargs=params)
