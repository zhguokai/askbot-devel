...
BUG_RE = re.compile(r'(?<![[/])WIND(\d\d\d\d\d\d\d\d)\b')
TSR_RE = re.compile(r'(?<![[/])TSR-?(\d\d\d\d\d+)\b')

LINK_PATTERNS = [
    (URL_RE, r'\1'),
    (BUG_RE, django_settings.BUG_DB_LINK + r'\1'),
    (TSR_RE, django_settings.TSR_DB_LINK + r'\1'),
]
