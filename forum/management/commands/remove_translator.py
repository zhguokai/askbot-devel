...
class Command(NoArgsCommand):
    ...
    def confirm_action(self):
        u = self.user
        print ''
        prompt = 'Do you really wish to REMOVE user (id=%d, name=%s) from the list of translators? yes/no: ' \
                % (u.id, u.username)
    ...
    def handle(self, *arguments, **options):
        ...
        tg = Group.objects.get(name='translators')
        self.user.groups.remove(tg)
        self.user.save()
